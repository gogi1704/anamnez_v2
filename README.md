...
Персональные ссылки из ИКП открываются по адресу `/ikp/<token>`. Для каждого ИНН
создаётся одна постоянная ссылка без технического срока действия. Переходы,
пользователи, прохождение стартового сценария, диалоги и сообщения показываются
в отдельном разделе **«ИКП»** админ-панели. Эта аудитория исключена из дашборда,
«Воронки и поведения», «Метрики 2.0» и счётчика Яндекс Метрики.

Для серверной интеграции задайте одинаковое случайное значение минимум из 32
символов: `IKP_INTEGRATION_SECRET` в `.env` «Консилиума» и
`CONSILIUM_INTEGRATION_SECRET` в `.env.production` проекта ИКП. Контейнер ИКП
должен быть подключён к внешней сети `consilium-internal` и обращаться по адресу
`CONSILIUM_API_ORIGIN=http://consilium:8000`.
...
Для отправки обезличенных отчётов воронки в чат с Bitrix AI добавьте в `.env`
Консилиума отдельный секрет:

```dotenv
BITRIX_METRICS_SECRET=ещё-одна-случайная-строка-не-короче-32-символов
FUNNEL_MONITOR_POLL_SECONDS=30
```

В `.env` проекта `bitrix_connector` это же значение задаётся как
`CONSILIUM_METRICS_SECRET`. Целевой чат можно выбрать в админке Консилиума;
необязательный `BITRIX_METRICS_DIALOG_ID` в коннекторе имеет приоритет над
выбором из админки. Секрет мониторинга должен отличаться от секрета платежей.
