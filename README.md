...
Пока боты не созданы, оставьте `TELEGRAM_BOT_AUTH_URL` и `MAX_BOT_AUTH_URL`
пустыми. После создания Telegram-бота добавьте, например:

```dotenv
TELEGRAM_BOT_AUTH_URL=https://t.me/имя_бота?start={token}
```

Для MAX укажите полный deep-link вашего бота с параметром `{token}` по
актуальной документации MAX. После изменения `.env` выполните:
...
### Контракт для будущих Telegram- и MAX-ботов
...
личные учётные записи, меняет имена и пароли, включает и отключает доступ.
