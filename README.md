...
скопируйте только его номер. В настройках счётчика не включайте Вебвизор,
аналитику форм и запись полей. Рекомендуется включить маскирование IP и принимать
данные только с указанных адресов сайта. После изменения `.env` пересоздайте
контейнер проекта.
...
Вебвизор и аналитика форм в коде приложения принудительно выключены: они могут
записывать медицинскую анкету, чат и результаты анализов. Карта кликов работает
без Вебвизора.
