...
Пока `ONLINE_PAYMENTS_ENABLED=false`, кнопка «Оплатить онлайн» показывает
информационную заглушку и новые платежи в ЮKassa не создаются. Код интеграции,
проверка ранее созданных платежей и обработчик уведомлений остаются в проекте.
Когда платёжный магазин будет готов, замените значение на `true` и пересоберите
контейнер Консилиума.
